  ...
  render() {
    ...
    this.state.players.map((player) => {
      user_display.push(
        <Typography key={player.name}>
          <Avatar alt={player.name} src={player.avatar_url} component="span" />
          {player.name}
          <br />
        </Typography>
      );
    ...
    return (
      <div>
        ...
        {user_display.length > 0 ? (
          <div>
            <Button color="secondary" onClick={() => this.client.game_leave()}>
              leave lobby
            </Button>
            <Button color="primary" onClick={() => this.client.game_start()}>
              Start Game
            </Button>
          </div>
        ) : (
